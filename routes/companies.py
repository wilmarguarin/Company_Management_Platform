...
def list_companies():
    ...
    for company in companies:
        ...
        company_dict['comment_count'] = conn.execute(
            "SELECT COUNT(*) FROM comments WHERE company_id = ?", (company_dict['id'],)).fetchone()[0]
...
def company_detail(company_id):
    ...
    if request.method == 'POST':
        ...
        conn.execute(
            "INSERT INTO comments (company_id, user, comment) VALUES (?, ?, ?)",(company_id, user, comment))
        conn.commit()
        conn.close()
...
def edit_company(company_id):
    ...
    if request.method == 'POST':
        ...
        existing_company = conn.execute("SELECT id FROM companies WHERE name = ? AND id != ?", (new_name, company_id)).fetchone()
